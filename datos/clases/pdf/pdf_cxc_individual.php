...
$pdf->Cell(80	,5,'CLIENTE',1,0);
$pdf->Cell(20	,5,'CUENTA',1,0);
$pdf->Cell(30	,5,'SALDO',1,0);
$pdf->Cell(30	,5,'TOTAL',1,0);
$pdf->Cell(35	,5,'FECHA DESDE',1,0);
$pdf->Cell(189	,10,'',0,1);//end of line
$pdf->SetFont('Arial','',12);

$query = DBSTART::abrirDB()->prepare("SELECT t1.id, t2.nombres, t1.total, t1.saldo, t1.fecha_cxc FROM c_cxc t1 INNER JOIN c_clientes t2 ON t1.cedula = t2.cedula
                                         WHERE t1.cedula = '$cliente' AND t1.estado = 'DEBE'");
$query->execute();
$all = $query->fetchAll(PDO::FETCH_ASSOC);
    foreach((array) $all as $item){
    	$pdf->SetFont('Arial','',8);
    	$pdf->Cell(80	,5,strtoupper($item['nombres']),0,0);
        $pdf->Cell(20	,5,$item['id'],0,0);
        $pdf->Cell(30	,5,$item['saldo'],0,0);
        $pdf->Cell(30	,5,$item['total'],0,0);
    	$pdf->Cell(35	,5,$item['fecha_cxc'],0,0);
