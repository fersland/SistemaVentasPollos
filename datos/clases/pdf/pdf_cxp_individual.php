...
class PDF extends FPDF{
    function Header(){
        ...
        $this->Cell(80);
        // Title
        $this->Cell(0,10,'Lista de Cuentas x Pagar',1,0,'C');
...
$query = DBSTART::abrirDB()->prepare("SELECT t2.nombreproveedor, t1.total, t1.saldo, t1.fecha_cxp FROM c_cxp t1 
                                                INNER JOIN c_proveedor t2 ON t1.id_proveedor = t2.id_proveedor
                                         WHERE t1.estado = 'DEBE'");
$query->execute();
$all = $query->fetchAll(PDO::FETCH_ASSOC);
    foreach((array) $all as $item){
    	$pdf->SetFont('Arial','',8);
    	$pdf->Cell(90	,5,strtoupper($item['nombreproveedor']),0,0);
        $pdf->Cell(30	,5,$item['saldo'],0,0);
        $pdf->Cell(30	,5,$item['total'],0,0);
    	$pdf->Cell(40	,5,$item['fecha_cxp'],0,0);
