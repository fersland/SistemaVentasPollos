...
$pdf->Cell(59	,5,'GASTOS DE EMPRESA MENSUAL',0,1);//end of line
$pdf->Cell(189	,10,'',0,1);//end of line
...
$pdf->Cell(55	,5,'TIPO',1,0);
$pdf->Cell(20	,5,'VALOR',1,0);
$pdf->Cell(45	,5,'FECHA',1,0);
$pdf->Cell(189	,10,'',0,1);//end of line
$pdf->SetFont('Arial','',14);

$query = DBSTART::abrirDB()->prepare("SELECT p.valor, p.fecha, tg.nombre as tipogastos
                        FROM c_resumen_gasto p
                        INNER JOIN c_tipo_gastos tg ON p.tipo = tg.id
                            WHERE p.anio = '$anio' and p.mes='$mes' and p.sucursal = '$sucursal'");
$query->execute();
$all = $query->fetchAll(PDO::FETCH_ASSOC);
foreach((array) $all as $item){
    	$pdf->SetFont('Arial','',10);
    	$pdf->Cell(55	,5,$item['tipogastos'],0,0);
    	$pdf->Cell(20	,5,$item['valor'],0,0);
    	$pdf->Cell(45	,5,$item['fecha'],0,0);
    	
    	$pdf->Cell(189	,0,'',0,1);//end of line
    	$pdf->Cell(100	,8,'  _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _',0,1);//end of line
}
...
$querytotally = DBSTART::abrirDB()->prepare("SELECT sum(p.valor) as total FROM c_resumen_gasto p WHERE p.anio = '$anio' and p.mes='$mes' and p.sucursal='$sucursal' ");
...
$pdf->Cell(220    ,0,'TOTAL GASTADO ESTE MES: ',0,1,'C');$pdf->Cell(325  ,0,number_format($send,2),0,1,'C');
