...
$pdf->Cell(59	,5,'Gastos Por Empleado - AÑO',0,1);//end of line
$pdf->Cell(189	,10,'',0,1);//end of line
...
$pdf->Cell(60	,5,'PERSONAL',1,0);
$pdf->Cell(55	,5,'TIPO',1,0);
$pdf->Cell(20	,5,'VALOR',1,0);
$pdf->Cell(35	,5,'FECHA_Y_HORA',1,0);
$pdf->Cell(189	,10,'',0,1);//end of line
...
$query = DBSTART::abrirDB()->prepare("SELECT r.cedula, concat(r.nombres, r.apellidos) epl, p.valor, p.fecha, pd.nombre as tipo
                        FROM c_resumen_gasto p
                            ...
                            WHERE p.id_empleado = '$empleado' and p.anio = '$anio' and p.sucursal='$sucursal'");
$query->execute();
$all = $query->fetchAll(PDO::FETCH_ASSOC);
foreach((array) $all as $item){
    	$pdf->SetFont('Arial','',10);
    	$pdf->Cell(30	,5,$item['cedula'],0,0);
    	$pdf->Cell(60	,5,$item['epl'],0,0);
    	$pdf->Cell(55	,5,$item['tipo'],0,0);
    	$pdf->Cell(20	,5,$item['valor'],0,0);
    	$pdf->Cell(35	,5,$item['fecha'],0,0);
    	
    	$pdf->Cell(189	,0,'',0,1);//end of line
    	$pdf->Cell(100	,8,' _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _',0,1);//end of line
}

$querytotally = DBSTART::abrirDB()->prepare("SELECT sum(p.valor) as total
                        FROM c_resumen_gasto p WHERE p.id_empleado = '$empleado' and p.anio = '$anio' and p.sucursal='$sucursal' ");
...
$pdf->Cell(240    ,0,'TOTAL GASTADO ESTE AÑO: ',0,1,'C');$pdf->Cell(325  ,0,number_format($send,2),0,1,'C');
