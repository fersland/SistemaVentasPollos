...
$pdf->Cell(59	,5,'Gastos Por Empleado - SEMANAL',0,1);//end of line
$pdf->Cell(189	,10,'',0,1);//end of line
...
$query = DBSTART::abrirDB()->prepare("SELECT p.id, r.cedula, r.nombres, r.apellidos, p.valor, p.fecha, pd.nombre as tipo
                        FROM c_resumen_gasto p
                            ...
                            WHERE p.id_estado = 1 AND p.sucursal = '$sucursal' AND p.id_empleado = '$emple' AND p.fecha_sin BETWEEN CURDATE() and CURDATE() + INTERVAL 7 DAY ");
$query->execute();
...
//SELECT sum(p.valor) as total FROM c_resumen_gasto p INNER JOIN c_empleados r ON p.id_empleado = r.id_empleado WHERE p.id_estado = 1 AND p.id_empleado = '$emple' AND fecha BETWEEN CURDATE() and CURDATE() + INTERVAL 7 DAY

$querytotally = DBSTART::abrirDB()->prepare("SELECT sum(p.valor) as total FROM c_resumen_gasto p 
                                                    INNER JOIN c_empleados r ON p.id_empleado = r.id_empleado 
                                                    WHERE p.id_estado = 1 
                                                    AND p.id_empleado = '$emple'
                                                    AND p.sucursal = '$sucursal'
                                                    AND p.fecha BETWEEN CURDATE() and CURDATE() - INTERVAL 7 DAY");
...
$pdf->Cell(230    ,0,'TOTAL GASTADO ESTA SEMANA: ',0,1,'C');$pdf->Cell(325  ,0,number_format($send,2),0,1,'C');
