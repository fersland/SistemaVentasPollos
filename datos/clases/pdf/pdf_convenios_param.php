...
class PDF extends FPDF{
    function Header(){
        ...
        $this->Cell(320  ,5,'Fecha:  '.date('Y-m-d'),0,0, 'C');
...
$pdf->Cell(70	,5,'PRODUCTO',1,0);
$pdf->Cell(30	,5,'INGRESO',1,0);
$pdf->Cell(20	,5,'CADUCA',1,0);
$pdf->Cell(189	,10,'',0,1);//end of line
$pdf->SetFont('Arial','',12);

$query = DBSTART::abrirDB()->prepare("SELECT t2.codproducto, t2.nombreproducto, t1.fecha_registro, t1.fechacaduca, t3.nombres FROM c_convenios t1 
                                        INNER JOIN c_mercaderia t2 ON t1.codigo = t2.codproducto
                                        INNER JOIN c_clientes t3 ON t1.cliente = t3.cedula
                                                WHERE t1.cliente='$cliente' AND t1.id_estado = 1");
$query->execute();
$all = $query->fetchAll(PDO::FETCH_ASSOC);
    foreach((array) $all as $item){
    	$pdf->SetFont('Arial','',8);
    	$pdf->Cell(25	,5,$item['codproducto'],0,0);
    	$pdf->Cell(50	,5,$item['nombres'],0,0);
    	$pdf->Cell(70	,5,$item['nombreproducto'],0,0);
    	$pdf->Cell(30	,5,$item['fecha_registro'],0,0);
        $pdf->Cell(20	,5,$item['fechacaduca'],0,0);
