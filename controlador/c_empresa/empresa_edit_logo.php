<?php
    ...
    try{
       ...
       if (in_array($ext, $formato)) { // CORRECTO

            if (move_uploaded_file($nombretmparchivo, "../../init/img/logo/$nombrearchivo")) {
		      $stmt = $db->prepare("UPDATE c_empresa SET img_pdf='$nombrearchivo'");
    		if ($stmt->execute()){
